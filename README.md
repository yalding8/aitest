...
### 4. 完善的授权机制
- **城市化授权码**：UHOMES + 城市代码 + 3位数字（如 UHOMESBJ123）
  - TJ（天津）、BJ（北京）、ZZ（郑州）、CD（成都）
  - 每个城市50个授权码，共200个
- **永久测试码**：UHOMES999（永不失效，仅用于测试）
- **使用追踪**：记录使用者姓名、邮箱、岗位、分数、使用时间
- **一次性有效**：授权码使用后自动失效（UHOMES999除外）
...
### 6. 考生操作指南
- **在线指南页面**：[https://test.pylosy.com/guide](https://test.pylosy.com/guide)
- 现代化UI设计，图标化信息展示
- 包含考前准备、登录步骤、考试规则、注意事项
- 可直接分享给考生，降低沟通成本

### 7. 自动化部署
- **GitHub Actions CI/CD**：代码推送后自动部署
- **GitLab 同步**：可选的代码镜像同步
- **一键部署脚本**：本地快速部署工具
...
2. **授权码管理**
   ...
   ```
   会生成两个 CSV 文件：
   - `授权码_已使用_YYYYMMDD.csv`：包含使用者详细信息
   - `授权码_未使用_YYYYMMDD.csv`：剩余可用授权码列表
...
- **考试入口**: [https://test.pylosy.com/](https://test.pylosy.com/)
- **操作指南**: [https://test.pylosy.com/guide](https://test.pylosy.com/guide)
...
### v2.2.0 (2026-01-08)
- 🔑 **授权码升级**：城市化授权码系统（UHOMES + 城市代码 + 数字）
- 📊 **使用追踪**：记录授权码使用者详细信息（姓名、邮箱、岗位、分数）
- 📁 **管理工具**：新增授权码导出脚本 `manager-export.sh`，生成 CSV 报表
- 📖 **考生指南**：新增在线操作指南页面 `/guide`，现代化UI设计
- 🚀 **自动化部署**：GitHub Actions CI/CD 自动部署
- 🔄 **GitLab 同步**：可选的代码镜像同步功能
- 🎨 **UI 优化**：指南页面采用图标化设计，提升用户体验
